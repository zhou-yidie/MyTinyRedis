...
RedisValue& RedisObject::operator[]( const std::string &key ) {
    std::_Rb_tree_iterator<std::pair<const std::basic_string<char>, RedisValue>>  it = value.find( key ) ;
    return ( ( it == value.end() )? staticNull() : it->second ) ;
}

RedisValue& RedisList::operator[](size_t i) {
    if( i >= ( size_t )( value.size() ) ){
        return staticNull() ;
    }
    else return value[ i ] ;
}
...
bool RedisValue::hasShape(const shape & types, std::string & err)  {
    ...
    // 获取 JSON 对象的所有成员项
    std::map<std::basic_string<char>, RedisValue> obj_items = objectItems() ;

    // 遍历指定的形状
    for (const std::pair<std::basic_string<char>, RedisValue::Type> & item : types) {
        // 查找 JSON 对象中是否存在形状中指定的项
        const  std::_Rb_tree_iterator<std::pair<const std::basic_string<char>, RedisValue>> it = obj_items.find(item.first);
